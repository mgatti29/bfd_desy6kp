...
// This time include convolution and removal of a PSF.
#include "Std.h"
#include "Moments.h"
#include "KWeight.h"
#include "Galaxy.h"
	 ...
	 char *argv[])
{
  ...
  try {
    ...
    bool failure = false;
    {
      ...
      cout << "Covariance numerical: " << pg1.getCov() << endl;
      /**/
    }

    MomentDerivs<> numeric = pg1.getDerivs();
    ...
    PixelGalaxy<> pg2(kw, data, map, psf2, noise * (dx*dx));

    cout << "---- Now using pixelated PSF---" << endl;
    numeric = pg2.getDerivs();
    ...
    cout << "---- Now shift PixelGalaxy origin to center---" << endl;
    TemplateGalaxy<>* pg3 = dynamic_cast<TemplateGalaxy<>*> (pg2.getShifted(-x0, -y0));
    GaussianGalaxy<> gal2(kw, flux, sigma*pow(1-e*e,-0.25), e, beta, 0., 0., noise);
    numeric = pg3->getDerivs();
    analytic = gal2.getDerivs();
