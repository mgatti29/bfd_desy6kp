...
numpy_version = np.float32(version_split_string[1])
...
class KSigmaWeight(object):
    ...
    def set_k(self, kx, ky):
        ...
        self.term = 1.0 - self.sigsq2 * (kx * kx + ky * ky) / self.n
        self.mask = self.term > 0.0
        self.w = np.where(self.mask, self.term**self.n, 0.0)
        self._dw = None
        self._d2w = None
    ...
    def dw(self):
        ...
        if self._dw is None:
            self._dw = (-self.sigsq2) * np.where(
                self.mask, self.term ** (self.n - 1), 0.0
            )
        return self._dw
    ...
    def d2w(self):
        ...
        if self._d2w is None:
            self._d2w = (
                self.sigsq2 * self.sigsq2 * (self.n - 1) / (self.n)
            ) * np.where(self.mask, self.term ** (self.n - 2), 0.0)
        return self._d2w
...
class KBlackmanHarris:
    ...
    def set_k(self, kx, ky):
        ...
        self._invkr = np.where(kr == 0, 1.0, 1 / kr).flatten()
    ...
    def dw(self):
        ...
        if self._dw is None:
            self._dw_du_mask = (
                -self.coeffs[1] * np.sin(self._u_mask)
            ...
            self._dw[self.mask_flat] = (
                (np.pi / (2 * self.kmax)) * self._dw_du_mask * self._invkr_mask
            )
    ...
    def d2w(self):
        ...
        if self._d2w is None:
            ...
            self._d2w_mask = (
                np.pi / (2 * self.kmax)
            ) ** 2 * d2w_du2 * self._invkr_mask * self._invkr_mask + (
                -0.25 * np.pi / self.kmax
            ) * self._dw_du_mask * self._invkr_mask * self._invkr_mask * self._invkr_mask
...
class Moment(object):
    ...
    def __init__(self, even=np.zeros(5, dtype=float), odd=np.zeros(2, dtype=float)):
...
class MomentCovariance(object):
    ...
    def __init__(self, even, odd=None):
        ...
        m = Moment()
        if not (
            len(even.shape) == 2 and even.shape[0] == m.NE and even.shape[1] == m.NE
        ):
        ...
        if odd is None:
            ...
            len(odd.shape) == 2 and odd.shape[0] == m.NO and odd.shape[1] == m.NO
        ):
    ...
    def isotropize(self):
        ...
        o[m.MX, m.MX] = o[m.MY, m.MY] = 0.5 * (
            self.odd[m.MX, m.MX] + self.odd[m.MY, m.MY]
        )
        return MomentCovariance(e, o)
...
class Template(object):
    ...
    def __init__(self, id=0, even=None, odd=None, nda=1.0, jSuppression=1.0):
        ...
        if even is not None and not (
            len(even.shape) == 2
            and even.shape[0] == self.NE
            and even.shape[1] == self.ND
        ):
            raise Exception("wrong shape for even derivative array")
        if odd is not None and not (
            len(odd.shape) == 2 and odd.shape[0] == self.NO and odd.shape[1] == self.ND
        ):
    ...
    def get_dmu(self):
        ...
        return self._to_moment(self.even[:, self.DU], self.odd[:, self.DU])

    def get_dg1(self):
        """Return derivs of moments wrt g1 shear"""
        return self._to_moment(
            (self.even[:, self.DV] + self.even[:, self.DVb]),
    ...
    def get_dg2(self):
        ...
        return self._to_moment(
            1j * (self.even[:, self.DV] - self.even[:, self.DVb]),
    ...
    def get_dmu_dg1(self):
        ...
        return self._to_moment(
            (self.even[:, self.DUDV] + self.even[:, self.DUDVb]),
    ...
    def get_dmu_dg2(self):
        ...
        return self._to_moment(
            1j * (self.even[:, self.DUDV] - self.even[:, self.DUDVb]),
    ...
    def get_dg1_dg1(self):
        ...
        return self._to_moment(
            2 * self.even[:, self.DVDVb]
            + self.even[:, self.DVDV]
            + self.even[:, self.DVbDVb],
            2 * self.odd[:, self.DVDVb]
            + self.odd[:, self.DVDV]
    ...
    def get_dg1_dg2(self):
        ...
        return self._to_moment(
            1j * (self.even[:, self.DVDV] - self.even[:, self.DVbDVb]),
...
class MomentCalculator(object):
    ...
    def __init__(self, kdata, weight, id=0, nda=1.0):
        """
        ...
        # included
        self._kval = (kdata.kval * np.where(kdata.conjugate, 2.0, 1.0))[
            self.weight.mask
        ] * kdata.d2k
        ...
        if kdata.kvar is None:
            ...
            self._kvar = (kdata.kvar * np.where(kdata.conjugate, 2.0, 1.0))[
                self.weight.mask
            ] * (kdata.d2k * kdata.d2k)
        self._ksq = None  # |k^2|
    ...
    def get_shotnoise(self, xVar, eta=1.0, dx=0.0, dy=0.0):
        ...
        self._kpsf[1::2, ::2] *= -1.0
        self._kpsf[::2, 1::2] *= -1.0
        ...
        xVarNew[
            (NNew - N) // 2 : -(NNew - N) // 2, (NNew - N) // 2 : -(NNew - N) // 2
        ] = xVar
        ...
        even = (
            ...
            * self._d2k**2
            / eta
            * np.sum(
                xw_psf[:, np.newaxis, :, :] * xw_psf[np.newaxis, :, :, :] * xVarNew,
                axis=(2, 3),
            )
        )
    ...
    def get_template(self, dx, dy):
        ...
        w0_terms_e = (
            ...
            (t.MR, t.DUDU, 6.0, 0, 2),
            (t.MR, t.DUDV, 2.0, -2, 2),
            (t.MR, t.DUDVb, 2.0, 2, 2),
            (t.MR, t.DVDVb, 2.0, 0, 2),
            (t.MC, t.D0, 1.0, 0, 4),
            (t.MC, t.DU, -4.0, 0, 4),
            (t.MC, t.DV, -2.0, -2, 4),
            (t.MC, t.DVb, -2.0, 2, 4),
            (t.MC, t.DUDU, 20.0, 0, 4),
            (t.MC, t.DUDV, 8.0, -2, 4),
            (t.MC, t.DUDVb, 8.0, 2, 4),
            (t.MC, t.DVDV, 2.0, -4, 4),
            (t.MC, t.DVDVb, 6.0, 0, 4),
            (t.MC, t.DVbDVb, 2.0, 4, 4),
            (t.ME, t.D0, 1.0, 2, 2),
            (t.ME, t.DU, -2.0, 2, 2),
            (t.ME, t.DV, -2.0, 0, 2),
            (t.ME, t.DUDU, 6.0, 2, 2),
        ...
        w0_terms_o = (
            ...
            (t.MX, t.DU, -1.0j, 1, 1),
            (t.MX, t.DV, -1.0j, -1, 1),
            (t.MX, t.DUDU, 2.0j, 1, 1),
            (t.MX, t.DUDV, 1.0j, -1, 1),
        ...
        w1_terms_e = (
            ...
            (t.M0, t.DUDU, 6.0, 0, 2),
            (t.M0, t.DUDV, 2.0, -2, 2),
            (t.M0, t.DUDVb, 2.0, 2, 2),
            (t.M0, t.DVDVb, 2.0, 0, 2),
            (t.MR, t.DU, -2.0, 0, 4),
            (t.MR, t.DV, -1.0, -2, 4),
            (t.MR, t.DVb, -1.0, 2, 4),
            (t.MR, t.DUDU, 14.0, 0, 4),
            (t.MR, t.DUDV, 6.0, -2, 4),
            (t.MR, t.DUDVb, 6.0, 2, 4),
            (t.MR, t.DVDV, 2.0, -4, 4),
            (t.MR, t.DVDVb, 4.0, 0, 4),
            (t.MR, t.DVbDVb, 2.0, 4, 4),
            (t.MC, t.DU, -2.0, 0, 6),
            (t.MC, t.DV, -1.0, -2, 6),
            (t.MC, t.DVb, -1.0, 2, 6),
            (t.MC, t.DUDU, 22.0, 0, 6),
            (t.MC, t.DUDV, 10.0, -2, 6),
            (t.MC, t.DUDVb, 10.0, 2, 6),
            (t.MC, t.DVDV, 4.0, -4, 6),
            (t.MC, t.DVDVb, 6.0, 0, 6),
            (t.MC, t.DVbDVb, 4.0, 4, 6),
            (t.ME, t.DU, -2.0, 2, 4),
            (t.ME, t.DV, -1.0, 0, 4),
            (t.ME, t.DVb, -1.0, 4, 4),
            (t.ME, t.DUDU, 14.0, 2, 4),
            (t.ME, t.DUDV, 10.0, 0, 4),
            (t.ME, t.DUDVb, 4.0, 4, 4),
            (t.ME, t.DVDV, 4.0, -2, 4),
            (t.ME, t.DVDVb, 4.0, 2, 4),
        )
        ...
        w1_terms_o = (
            ...
            (t.MX, t.DV, -1.0j, -1, 3),
            (t.MX, t.DVb, -1.0j, 3, 3),
            (t.MX, t.DUDU, 10.0j, 1, 3),
            (t.MX, t.DUDV, 5.0j, -1, 3),
            (t.MX, t.DUDVb, 3.0j, 3, 3),
            (t.MX, t.DVDV, 2.0j, -3, 3),
            (t.MX, t.DVDVb, 3.0j, 1, 3),
        )
        ...
        w2_terms_e = (
            (t.M0, t.DUDU, 4.0, 0, 4),
            (t.M0, t.DUDV, 2.0, -2, 4),
            (t.M0, t.DUDVb, 2.0, 2, 4),
            (t.M0, t.DVDV, 1.0, -4, 4),
            (t.M0, t.DVDVb, 1.0, 0, 4),
            (t.M0, t.DVbDVb, 1.0, 4, 4),
            (t.MR, t.DUDU, 4.0, 0, 6),
            (t.MR, t.DUDV, 2.0, -2, 6),
            (t.MR, t.DUDVb, 2.0, 2, 6),
            (t.MR, t.DVDV, 1.0, -4, 6),
            (t.MR, t.DVDVb, 1.0, 0, 6),
            (t.MR, t.DVbDVb, 1.0, 4, 6),
            (t.MC, t.DUDU, 4.0, 0, 8),
            (t.MC, t.DUDV, 2.0, -2, 8),
            (t.MC, t.DUDVb, 2.0, 2, 8),
            (t.MC, t.DVDV, 1.0, -4, 8),
            (t.MC, t.DVDVb, 1.0, 0, 8),
            (t.MC, t.DVbDVb, 1.0, 4, 8),
            (t.ME, t.DUDU, 4.0, 2, 6),
            (t.ME, t.DUDV, 2.0, 0, 6),
            (t.ME, t.DUDVb, 2.0, 4, 6),
            (t.ME, t.DVDV, 1.0, -2, 6),
            (t.ME, t.DVDVb, 1.0, 2, 6),
            (t.ME, t.DVbDVb, 1.0, 6, 6),
        )

        w2_terms_o = (
            (t.MX, t.DUDU, 4.0j, 1, 5),
            (t.MX, t.DUDV, 2.0j, -1, 5),
            (t.MX, t.DUDVb, 2.0j, 3, 5),
            (t.MX, t.DVDV, 1.0j, -3, 5),
            (t.MX, t.DVDVb, 1.0j, 1, 5),
            (t.MX, t.DVbDVb, 1.0j, 5, 5),
        )
        ...
        for wt, terms_e, terms_o in (
            (self.weight.w[mask], w0_terms_e, w0_terms_o),
            ...
            for term in terms_e:
                if term[3] >= 0:
                    even_out[term[0], term[1]] += (
                        term[2] * mN[term[3] // 2, term[4] // 2]
                    )
                else:
                    # Negative m's must conjugate the multipole
                    even_out[term[0], term[1]] += term[2] * np.conj(
                        mN[(-term[3]) // 2, term[4] // 2]
                    )
    ...
    def xy_jacobian(self, dx):
        ...
        e = m.even
        return -0.5 * np.array(
            [[e[m.MR] + e[m.M1], e[m.M2]], [e[m.M2], e[m.MR] - e[m.M1]]]
        )
    ...
    def recenter(self):
        ...
        dx, junk, ier, msg = fsolve(
            self.xy_moment, dx, fprime=self.xy_jacobian, full_output=True, maxfev=500
        )
    ...
    def make_templates(
        ...
        **kwargs
        ...
        if error:
            return (
                False,
                "Center wandered too far from starting guess or failed to converge",
                None,
                None,
            )
...
def xyWin(psf, sigma, nominal=None, nIter=3):
    ...
    for i in range(nIter):
        y, x = np.meshgrid(
            (xx - center[0]) / sigma, (yy - center[1]) / sigma, indexing="ij"
        )
        ...
        d2yx = np.array(
            [
                [np.sum(psf * wt * (y * y - 1)), np.sum(psf * wt * x * y)],
...
def simpleImage(
    ...
    if numpy_version >= 8:
        ky = (
            np.ones((N, N // 2 + 1), dtype=float)
            * np.fft.fftfreq(N)[:, np.newaxis]
            * 2.0
            * np.pi
        )
        kx = np.ones((N, N // 2 + 1), dtype=float) * np.fft.rfftfreq(N) * 2.0 * np.pi
    else:
        ky = (
            np.ones((N, N // 2 + 1), dtype=float)
            * np.fft.fftfreq(N)[:, np.newaxis]
        ...
        kxtmp[-1] *= -1
        kx = np.ones((N, N // 2 + 1), dtype=float) * kxtmp * 2.0 * np.pi
    # K area per sample
    d2k = (kx[0, 1] - kx[0, 0]) ** 2
...
def simpleImageCross(
    ...
    if (
        psf1.shape != image1.shape
        or psf1.ndim != 2
        or image1.shape[0] != image1.shape[1]
    ):
        raise Exception(
            "PSF1 and image1 shape must be matching square arrays for simpleImageCross"
        )

    if (
        psf2.shape != image2.shape
        or psf2.ndim != 2
        or image2.shape[0] != image2.shape[1]
    ):
        raise Exception(
            "PSF2 and image2 shape must be matching square arrays for simpleImageCross"
        )
    # ??? Do zero padding to some nominal size(s)
    ...
    if pad_factor > 1:
        ...
        Nnew = N * pad_factor
        Onew = Nnew / 2.0
        llimnew = int(Onew - Norig / 2.0)
        hlimnew = int(Onew + Norig / 2.0)
        ...
        if wcs1 is None:
            ...
            wcs1.xy0 = wcs1.xy0 - (Norig / 2.0) + (Nnew / 2.0)

        if wcs2 is None:
            origin = np.array([Onew, Onew])
        else:
            wcs2 = copy.deepcopy(wcs2)
            wcs2.xy0 = wcs2.xy0 - (Norig / 2.0) + (Nnew / 2.0)
    ...
    if numpy_version >= 8:
        ky1 = (
            np.ones_like(kpsf1, dtype=float)
            * np.fft.fftfreq(N)[:, np.newaxis]
            * 2.0
            * np.pi
        )
        kx1 = np.ones_like(kpsf1, dtype=float) * np.fft.rfftfreq(N) * 2.0 * np.pi

        ky2 = (
            np.ones_like(kpsf2, dtype=float)
            * np.fft.fftfreq(N)[:, np.newaxis]
            * 2.0
            * np.pi
        )
        kx2 = np.ones_like(kpsf2, dtype=float) * np.fft.rfftfreq(N) * 2.0 * np.pi
    else:
        ky1 = (
            np.ones_like(kpsf1, dtype=float)
            * np.fft.fftfreq(N)[:, np.newaxis]
        ...
        kx1tmp[-1] *= -1
        kx1 = np.ones_like(kpsf1, dtype=float) * kx1tmp * 2.0 * np.pi
        ...
        kx2tmp[-1] *= -1
        kx2 = np.ones_like(kpsf2, dtype=float) * kx2tmp * 2.0 * np.pi
    # K area per sample
