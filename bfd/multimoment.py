...
from .momentcalc import KData, Moment, MomentCalculator, Template, simpleImage
...
class MultiMomentCalculator(MomentCalculator):
    ...
    def __init__(self, kdata, weight, id=0, nda=1.0, img_weights=None, bandinfo=None):
        """
        ...
        if allnone:
            ...
            if nonone:
                ...
                if bandinfo is None:
                    raise Exception(
                        "Must provide Band info if providing multiple bands"
                    )
                else:
                ...
                raise Exception(
                    "If giving more than one band, cannot have None in band list"
                )

        # set up a series of MomentCalculators
        self.mc = [
            MomentCalculator(kd, self.weight, self.id, self.nda) for kd in self.kdata
        ]

        if img_weights is None:
            self.img_weights = self.get_img_weights()
        else:
            if len(img_weights) != len(kdata):
                raise Exception(
                    "MultiMomentCalculator weight length does not match # of images"
                )
            else:
    ...
    def get_img_weights(self):
        ...
        if no_weight:
            wt = np.array([1.0 for mc in self.mc])
        else:
            wt = np.array([1.0 / mc.get_covariance()[0][m.M0][m.M0] for mc in self.mc])
    ...
    def get_template(self, dx, dy, returnbands=False):
        ...
        if self.bandinfo is not None:
            ...
            for kk in range(self.nbands):
                ...
                tband[kk] = Template(
                    self.id, even_out_band[:, :, b], odd_out_band[:, :, b], nda=self.nda
                )
...
def multiImage(
    ...
    for ii, img in enumerate(imagelist):
        ...
        kdata_, psf_shift = simpleImage(
            ...
            band=band,
            psf_recenter_sigma=psf_recenter_sigma,
        )
