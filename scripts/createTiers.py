...
def createTiers(targetFile=None,
    ...
    for f in targetFile:
        ...
        hdu = pf.open(f)[1]
        if wtN is None:
            wtN = hdu.header[bfd.hdrkeys['weightN']]
        elif hdu.header[bfd.hdrkeys['weightN']]!=wtN:
            raise ValueError(bfd.hdrkeys['weightN'],'in file ' + f + ' does not match')
        if wtSigma is None:
            wtSigma = hdu.header[bfd.hdrkeys['weightSigma']]
        elif hdu.header[bfd.hdrkeys['weightSigma']]!=wtSigma:
            raise ValueError(bfd.hdrkeys['weightSigma'], 'in file ' + f + ' does not match')

        # Save a random subsample of the covariances
        keep = np.random.random(size=len(hdu.data)) < sample
        covs.append(hdu.data[bfd.colnames['covariance']][keep])
