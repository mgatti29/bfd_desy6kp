#!/usr/bin/env python
'''
wrapper to run BFD on DES simulations
"A" simulation, postage-stamp, psf included in MEDS file, only one image/galaxy, in the first spot, no blending issues, multiple bands though
'''
import sys
import os
import math
...
def parse_input():
    ...
    parser.add_argument('--pad_factor',help='factor by which to zero pad the image array before measuring moments',type=int,default=1)
    ...
    group = parser.add_mutually_exclusive_group()
    group.add_argument('--skip_coadd', help='Skips coadd (first image) in MEDS file -OR-', action='store_const', const=True)
    group.add_argument('--keep_coadd', help='Keeps coadd (first image) in MEDS file', action='store_const', const=True)
    ...
    defaults = {'dir':"",
                ...
                'moffile':None,
                'pad_factor':1} # No default MOF file
    ...
    group.add_argument('--tmpl_target_file', help='target moment file name', type=str)
    group.add_argument('--tmpl_sn_min', help='Lower flux moment S/N cut on targets', type=float)
    group.add_argument('--tmpl_noise_factor', help='noise reduction of templates compared to targets', type=float)
    ...
    if args.skip_coadd is not None:
        params['skip_coadd'] = True
    elif args.keep_coadd is not None or 'skip_coadd' not in params:
...
def check_params(params):
    ...
    if 'target_file' in params['TEMPLATE'] and params['make_template']:
        ...
        if os.path.isfile(fitsfile):
            ...
            M0=bfd.Moment().M0
            # take covariance from first example (all the same for these sims)
            params['TEMPLATE']['sigma_xy']= np.sqrt(hdu[0].header['COVMXMX'])
            params['TEMPLATE']['sigma_flux']=np.sqrt(hdu[0].data[0,0])
            params['WEIGHT']['n']=hdu[0].header['WT_N']
            params['WEIGHT']['sigma']=hdu[0].header['WT_SIG']
        else:
...
def main(params):
    ...
    for i in xrange(numobjs):
        ...
        obji = meds.get_cutout_list(i,skip_coadd=skip_coadd)
 
        nims = len(obji)

        # dictionary of WCS jacobian
        wcsi = meds.get_jacobian_list(i, skip_coadd=skip_coadd)

        # background noise of image (sigma) 
        noisei = meds.get_noise_list(i, skip_coadd=skip_coadd)

        # list of PSF images
        psfi = meds.get_psf_list(i, skip_coadd=skip_coadd)
        ...
        for j in range(nims):
            ...
            if template:
                obji[j]/=params['TEMPLATE']['noise_factor']
            ...
            kdata.append( bfd.simpleImage(obji[j], origin, psfi[j], wcs=wcs, pad_factor=params['pad_factor'],pixel_noise=noisei[j]))


        m = bfd.MomentCalculator(kdata[0], wt, id = i, nda=1.0)
        ...
        if template:
            ...
            if not t:
                logging.warning("no templates measured for %sth galaxy" %(i))
            elif t[0] is None:
